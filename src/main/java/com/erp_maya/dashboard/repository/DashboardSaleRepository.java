...
public interface DashboardSaleRepository extends JpaRepository<Sale, Long> {
    ...
    @Query("SELECT COALESCE(SUM(s.total), 0), COUNT(s) FROM Sale s WHERE s.companyId = :companyId AND s.saleDate >= :from")
    List<Object[]> totalsSince(Long companyId, Instant from);

    /** [ saleDate, total ] de las ventas desde una fecha (para la serie diaria). */
    @Query("SELECT s.saleDate, s.total FROM Sale s WHERE s.companyId = :companyId AND s.saleDate >= :from ORDER BY s.saleDate")
    List<Object[]> amountsSince(Long companyId, Instant from);

    /** [ branchId, branchName, SUM(total) ] agrupado por sucursal desde una fecha. */
    @Query("SELECT s.branch.id, s.branch.name, SUM(s.total) FROM Sale s WHERE s.companyId = :companyId AND s.saleDate >= :from GROUP BY s.branch.id, s.branch.name ORDER BY SUM(s.total) DESC")
    ...
    @Query("SELECT s.docNumber, s.saleDate, s.branch.name, s.paymentMethod, s.total FROM Sale s WHERE s.companyId = :companyId ORDER BY s.saleDate DESC")
    ...
    @Query("SELECT s.branch.id, s.branch.name, COUNT(s), SUM(s.total) FROM Sale s WHERE s.companyId = :companyId AND s.saleDate >= :from GROUP BY s.branch.id, s.branch.name ORDER BY SUM(s.total) DESC")
    ...
    @Query("SELECT s.paymentMethod, SUM(s.total) FROM Sale s WHERE s.companyId = :companyId AND s.saleDate >= :from GROUP BY s.paymentMethod ORDER BY SUM(s.total) DESC")
